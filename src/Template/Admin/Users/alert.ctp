<div class="col-md-12">
    <div class="panel panel-primary panstyl">
        <div class="panel-heading clearfix">
            <h3 class="panel-title">Toutes les alertes</h3>
        </div>
                    ...
                    <tr>
                        <td>
                          <a href="<?= $this->Url->build(['controller' => 'Users', 'action'=>'view',
                           'prefix'=>false, $alert->user->id, $alert->user->login]); ?>" target="_blank">
                            <?= $alert->user->login ?></a>
                        </td>
                        <td><?= $alert->text ?></td>
                        <td>
                            <a href="<?= $this->Url->build([ 'controller' => 'Contests',
                             'action' => 'gameview',$alert->contest_id, strtolower(str_replace(' ', '-', removeAccents($alert->contest->name))), 'prefix'=>false]); ?>" target="_blank">
                                <button type="button" class="btn btn-sm btn-info">
                                    <i class="fa fa-external-link" aria-hidden="true"></i> Fiche
                                </button>
                            </a>
                            <?= $this->Form->postLink(__('<i class="fa fa-trash" aria-hidden="true"></i>'),
                            ['controller' => 'Users', 'action' => 'delalert', $alert->id],
